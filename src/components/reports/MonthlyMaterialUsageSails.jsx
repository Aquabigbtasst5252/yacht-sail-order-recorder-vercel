// src/components/reports/MonthlyMaterialUsageSails.jsx
import React, { useMemo, forwardRef, useRef, useImperativeHandle } from 'react';
import { Line } from 'react-chartjs-2';
import { format } from 'date-fns';

const MonthlyMaterialUsageSails = forwardRef(({ orders }, ref) => {
    const chartRef = useRef(null);

    const processedData = useMemo(() => {
        const materialUsage = orders
            .filter(order => order.orderTypeName === 'Sail' && order.material && order.orderDate)
            .reduce((acc, order) => {
        ...
        return { labels: formattedLabels, datasets, tableData, title: 'Monthly Material Usage (Sails)' };
    }, [orders]);
                        ...
                        <table className="table table-striped table-sm">
                            ...
                            <tbody>
                                {processedData.tableData.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan="3" className="text-center">No sail material usage in this period.</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
...
export default MonthlyMaterialUsageSails;